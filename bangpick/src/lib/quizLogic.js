 ...
 */
export function buildQuizPrompt(type, personality, scores) {
  const quiz = QUIZZES[type]
  const dimDesc = quiz.dimensions
    .map((d) => `${quiz.dimensionLabels[d]}: ${scores[d]}分`)
    .join('、')

  const context = type === 'decision' ? '决策风格' : '投资风格'

  return {
    system: `你是一个幽默毒舌的${context}分析师。用户刚做了一个${context}测试，你需要根据他的测试结果给出一段个性化、有趣、有梗的解读。
要求：
...
4. 只返回 JSON，不要其他内容`,
    messages: [
      {
        role: 'user',
        content: `测试类型：${quiz.title}
结果人格：${personality.name}（${personality.desc}）
各维度得分：${dimDesc}
请给出个性化解读。`,
      },
    ],
