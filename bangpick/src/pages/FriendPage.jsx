  ...
  if (!decision) {
    return (
      <div className="flex flex-col items-center justify-center h-dvh bg-slate-900 p-4">
        <p className="text-slate-400 mb-4">{t('friend.data_lost')}</p>
        <button onClick={() => navigate('/')} className="text-blue-400">{t('friend.back_home')}</button>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-dvh bg-slate-900">
      <header className="flex items-center gap-3 px-4 py-3 bg-slate-800 border-b border-slate-700">
        ...
        <h1 className="text-lg font-bold text-white flex-1 truncate">{t('friend.title')}</h1>
      </header>

      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="text-center">
          <p className="text-slate-400 text-sm">{t('friend.my_dilemma')}</p>
